...
currentAccount = m.prop(accounts['primary'])
...
var run = function() {
    ...
    function activeFileTab() {
        ...
        return $('#files .file').filter(function() {
    ...
    function setEditorSize(delta) {
    	...
    	else if (delta > 65) delta = 65;
        $('#editor').attr("flex", delta);
        onResize();
    }
    ...
    $(document).mouseup(function(e) {
        if (dragging) {
            var delta = round5(parseInt(e.pageX / $body.width() * 100));
            $('#ghostbar').remove();
    ...
    $('.toggleRHP').click(function() {
        hidingRHP = !hidingRHP;
        setEditorSize(hidingRHP ? 100 : window.localStorage[EDITOR_SIZE_CACHE_KEY]);
        $('#righthand-panel').attr("flex",hidingRHP ? 0 : 1);
